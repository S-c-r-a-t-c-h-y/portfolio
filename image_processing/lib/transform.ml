...
let rotate_angle (degrees : float) src =
  let shear angle x y =
...
let convolve kernel src =
  ...
  for x = 0 to padded_image.width - size_x do
    for y = 0 to padded_image.height - size_y do
      let window = crop x y size_x size_y padded_image in
      let red_matrix =
        Array.map
          (fun row ->
            Array.map
...
let gaussian_blur ?(sigma : float = -1.) kernel_size img =
  let kernel =
