...
class labyrinthe:
    ...
    def regenerer(self):
        ...
        for i in range(self.nb_lignes):
            for j in range(self.nb_colonnes):
                self.murs[i][j] = [
                    m.maze_map[(i + 1, j + 1)]["N"],
                    m.maze_map[(i + 1, j + 1)]["E"],
                    m.maze_map[(i + 1, j + 1)]["S"],
                    m.maze_map[(i + 1, j + 1)]["W"],
                ]
    ...
    def dessiner(self):
        ...
        setworldcoordinates(
            -1, -(self.nb_lignes * self.u + 1), self.nb_colonnes * self.u + 1, 1
        )
        t = Turtle()
    ...
    def dessiner_chemin(self, chemin=None):
        ...
        t.pensize(pensize)
        t.goto(
            chemin[0][1] * self.u + self.u // 2, -chemin[0][0] * self.u - self.u // 2
        )
        t.pendown()
        for i, j in chemin[1:]:
    ...
    def generer_labyrinthe(self, width, height):
        ...
        self.murs = [
            [[True, True, True, True] for _ in range(width)] for _ in range(height)
        ]
