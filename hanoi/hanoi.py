...
# IMPLEMENTE UN COMPTEUR DE DEPLACEMENT
...
class Hanoi(tk.Tk):
    ...
    def __init__(self, nb_disques=3):
        ...
        self.text = self.canvas.create_text(
            self.WIDTH / 2, 20, text="0", font=font, justify="center"
        )
    ...
    def deplacer(self, depart: int, arrive: int):
        ...
        x1, y1, x2, y2 = self.get_coords(
            arrive - 1, len(self.__tours[arrive - 1]) - 1, obj
        )

        self.canvas.coords(disque, x1, y1, x2, y2)
        self.canvas.itemconfigure(self.text, text=f"{self.compteur_deplacement}")
        self.update()
    ...
    def get_coords(self, tour: int, nb_hauteur: int, disque: int):
        ...
        largeur = interp(
            disque, [1, self.__nb_disques], [self.MIN_WIDTH, self.TOWER_WIDTH]
        )
